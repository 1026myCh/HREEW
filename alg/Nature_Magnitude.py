...
def cal_magnitude(Traces_evt_org, Distance, beM, Sprate, Debug, Stime, flagarea, PGAold, PGAnew, flag_Sfirst, in_model,
    ...
    if Stime <= 0 and Distance > 25:
        ...
        if lent >= 500:
            ...
            # 陶茨法计算震级
            mag_tc1, _ = TaoC(Traces_evt[2][0:int(2.5 * Sprate)], Sprate, 'sc', 2, 0)
            temp = [Magnitude11]
            ...
            if num > 3:
                ...
                Magnitude22 = (np.log10(d2 * 10000) + 1.2 * np.log10(Distance) +
                               5e-4 * Distance - 5e-3 * 10 + 0.46) / 0.72
                mag_tc2, _ = TaoC(Traces_evt[5][0:int(2.5 * Sprate)], Sprate, 'sc', 2, 0)
            ...
            mag_tc1, _ = TaoC(Traces_evt[2][:], Sprate, 'sc', 2, 0)
            minM = mag_tc1
            Magnitude2 = minM
            ...
            if num > 3:  # 双传感器
                ...
                temp = [Magnitude11, Magnitude22]
                Magnitude1 = np.min(temp)
                mag_tc2, _ = TaoC(Traces_evt[5][:], Sprate, 'sc', 2, 0)
                tempM = [mag_tc1, mag_tc2]
                minM = np.min(tempM)
                Magnitude2 = minM
        ...
        if result <= 3.5:
            mag_tc1, _ = TaoC(Traces_evt[2][:], Sprate, 'sc', 0, 0)
            mag_tc2, _ = TaoC(Traces_evt[5][:], Sprate, 'sc', 0, 0)
            result = np.mean([mag_tc1, mag_tc2])
        elif result > 5.5:
            # 大震特殊处理
            if Traces_evt.shape[1] <= 600:
                mag_tc2, _ = TaoC(Traces_evt[5][:], Sprate, 'sc', 1, 0)
                mag_tc1, _ = TaoC(Traces_evt[2][:], Sprate, 'sc', 1, 0)
                b, a = butter(2, 0.075 * 2 / 200, 'high')
                b1, a1 = butter(2, 15 * 2 / 200, 'low')
                ...
                # 长数据特殊处理
                mag_tc2, _ = TaoC(Traces_evt[5][0:int(3 * Sprate)], Sprate, 'sc', 1, 0)
                mag_tc1, _ = TaoC(Traces_evt[2][0:int(3 * Sprate)], Sprate, 'sc', 1, 0)
                b, a = butter(2, 0.075 * 2 / 200, 'high')
                b1, a1 = butter(2, 15 * 2 / 200, 'low')
