...
def Nature_Single(Data_now, StartT,MaxEEW_times, StationInfo, NewInfo, Debug, Sprate, ThreshGals, S_time2,
    ...
    # 条件结合新修改的地震状态标志修改！
    if Sta_vars1.Is_EQK >= 0 and Sta_vars1.SNout[0] == 0:
        SNoutj = [0, 0, 0]
        SNout = snr(Sta_vars1.Package,Sta_vars1.Buffer,Sta_vars1.StartT,Sta_vars1.P_time,2, flag=None)
        Sta_vars1.SNout = SNout
        # 相似性来自judge
        Sta_vars1.corrENZ = corrout
        Ef_UDj = ef_udcmp(traces_org)
