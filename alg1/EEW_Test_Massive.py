...
def EEW_Test_Massvie(datadirRRR, excelName=None, Num=0):
    ...
    for k in range(0, Nfile,  group_size):
        ...
        for ttt in range(L - 1):
            ...
            if (NewInfo > 0 or (nature_mode == 1 and getattr(Sta_vars_ret1, 'Is_EQK', -1) == 0)) and  StationInfo.P_time>0:  # NewInfo == 1 or NewInfo == 3 or NewInfo == 2:
                ...
                Sta_vars_ret1.SNout[0],Sta_vars_ret1.SNout[1],Sta_vars_ret1.SNout[2],
                ef11[0],ef11[1],ef11[2],ef11[3],ef11[4],ef11[5],ef11[6],ef11[7],ef11[8],ef11[9],ef11[10],
                Sta_vars_ret1.flagarea,Sta_vars_ret1.PGD_Curr,Sta_vars_ret1.LenCh6,Sta_vars_ret1.TC,Sta_vars_ret1.sAvghm,Sta_vars_ret1.Is_EQK)
    ...
    heads = ['time_start', 'epi_No.', 'EQ_Info', 'STA_Name', 'Epi_time', 'epiLon_real', 'epiLat_real', 'Magnitude_real',
             ...
             'snre','snrn','snrz',
             'efud1','efud2','efud3','efud4','efud5','efud6','efud7','efud8','efud9','efud10','efud11',
             'FlagArea','PGD_Curr','LenCh6','TCmean','sAvghm','isEqk']
    ws = wb.active
    Title_len = len(heads)
    max_len = ws.max_row
    title_col = 1
    ...
    max_len = ws.max_row
    # deltline=
    for rows in range(max_len + 1, max_len + L ):
