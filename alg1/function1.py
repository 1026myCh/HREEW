...
from scipy.signal import lfiltic, lfilter_zi, lfilter
import numpy as np
...
def snr(Package,Buffer,StartT,P_time,mod=2):
    ...
    # 3. 基础参数计算：数据长度与时间差
    LP = len(Package[1])  # Package的长度（原MATLAB length函数）
    BL = Buffer.shape[1]  # Buffer的行数（原MATLAB length(Buffer)）
    ...
    # --------------------------
        ...
        plen = int(deltatime * 200 + LP)  # 信号数据总长度（点）
        pind = int(BL - plen)  # 信号起始索引（Buffer的行索引）
        pend = int(pind + 200 * 2 - 1)  # 信号结束索引（2秒信号：200*2点）
        pbef = int(pind - 200 * 5 - 1)  # 噪声起始索引（5秒噪声：200*5点，在信号前）
...
def ef_udcmp(traces_org, sprate):
    """
    计算UD（垂直）方向多频段能量分布
    功能：对输入数据的指定列执行傅里叶变换，按5Hz间隔分割0-55Hz频段，计算每段能量均值：
          - 若频段部分存在（仅部分频率点在序列中），用现有频率点的能量计算均值
          - 若频段完全缺失（无任何频率点在序列中），对应段结果填0
    参数：
        traces_org: 原始数据矩阵，shape=(n_samples, 6)，6列分别对应EW1、NS1、UD1、EW2、NS2、UD2
        sprate: 采样率（Hz），此处固定为200Hz（与傅里叶变换函数适配）
    返回：
        ef_ud: UD方向各频段能量数组，shape=(11,)，对应0-5,5-10,...,50-55Hz共11个频段
    """
    # -------------------------- 1. 输入合法性检查 --------------------------
    # 避免因输入数据列数不足导致后续索引报错
    traces_org = traces_org.T
    if traces_org.shape[1] < 6:
        raise ValueError(f"输入数据需至少6列（对应EW1/NS1/UD1/EW2/NS2/UD2），当前仅{traces_org.shape[1]}列")
    if sprate <= 0:
        raise ValueError(f"采样率需为正数，当前为{sprate}Hz")

    # -------------------------- 2. 傅里叶变换（提取UD方向数据） --------------------------
    ud_cols = [2, 5]  # UD1对应第3列（Python索引2），UD2对应第6列（Python索引5）
    # 执行傅里叶变换，确保返回格式为 (频率序列, 振幅谱)，shape均为(2, n_freq)
    freq_ud_list = [Fourier(traces_org[:, col], sprate) for col in ud_cols]
    freq_ud1, freq_ud2 = freq_ud_list  # 解包两个UD通道的频域数据
    freq_seq = freq_ud1[0]  # 频率序列（所有UD通道频率序列一致，取其一即可）
    amp1 = freq_ud1[1]      # UD1通道振幅谱
    amp2 = freq_ud2[1]      # UD2通道振幅谱

    # -------------------------- 3. 定义频段配置（0-55Hz，每5Hz一段） --------------------------
    freq_start = 0    # 总起始频率
    freq_end = 55     # 总终止频率
    freq_step = 5     # 频段宽度
    # 生成11个频段的边界（左闭右开）：[(0,5), (5,10), ..., (50,55)]
    bands = [(i * freq_step, (i + 1) * freq_step) for i in range(int((freq_end - freq_start) / freq_step))]

    # -------------------------- 4. 逐频段计算能量（核心逻辑） --------------------------
    ef_ud = []
    for band_low, band_high in bands:
        # 步骤1：找到当前频段在频率序列中的索引（左闭右开，匹配频段范围）
        # 例如：频段(5,10)对应频率序列中 5≤freq<10 的所有索引
        band_mask = (freq_seq >= band_low) & (freq_seq < band_high)
        band_indices = np.where(band_mask)[0]  # 满足频段范围的频率点索引

        # 步骤2：处理“频段完全缺失”场景（无任何频率点在当前频段内）
        if len(band_indices) == 0:
            ef_ud.append(0.0)
            continue

        # 步骤3：处理“频段部分存在”场景（有频率点在当前频段内）
        # 提取两个UD通道在当前频段内的振幅谱，计算能量和
        energy_ud1 = np.sum(amp1[band_indices])  # UD1通道当前频段能量
        energy_ud2 = np.sum(amp2[band_indices])  # UD2通道当前频段能量
        # 计算两通道能量均值（若单通道数据异常，可加异常值过滤，此处保持原逻辑）
        band_energy = (energy_ud1 + energy_ud2) / 2
        ef_ud.append(band_energy)

    # -------------------------- 5. 格式转换与返回 --------------------------
    return np.array(ef_ud, dtype=np.float64)  # 固定浮点类型，避免类型不一致


# def cmp4(Traces_evt, dist, Sprate):
#     """
#     功能：计算信号长度、特征参数TC、平均位移P_ud_m、平均最大位移sAvghm
#     Parameters:
#         Traces_evt (np.ndarray): 输入信号矩阵（行数=采样点，列数≥6，对应原代码的6个通道）
#         dist (float): 距离参数（传递给TaoC函数）
#         Sprate (np.ndarray): 采样率（原代码中用Sprate(1)，需确保为1D数组）
#     Returns:
#         lenCh6 (int): 第3通道（索引2）的信号长度
#         TC (float): 特征参数（两次TaoC结果的平均值）
#         P_ud_m (float): 第3/6通道的平均最大位移
#         sAvghm (float): 4个通道（1/4/2/5）的平均最大位移
#     """
#     # --------------------------
#     # 1. 初始化参数
#     # --------------------------
...
#     sAvghm = 0.0
#
#     # 检查第3通道（索引2）的行数是否小于采样率，不足则直接返回初始值
#     ch3 = Traces_evt[:, 2]  # Python索引从0开始，原MATLAB的(:,3)对应(:,2)
#     ch6 = Traces_evt[:, 5]
#     if len(ch3) < Sprate[0]:
#         return lenCh6, TC, P_ud_m, sAvghm
#
#     # 记录第3通道的长度（原代码lenCh6）
#     lenCh6 = len(ch3)
#
...
#     # 2. 计算TC（两次TaoC结果的平均值）
#     # --------------------------
#     # 原代码中两次调用TaoC参数完全一致，故直接计算两次并取平均
#     # mag_tc1, tc1 = TaoC(ch3, Sprate[0], 'sc', 2, dist, 0)
#     # mag_tc2, tc2 = TaoC(ch6, Sprate[0], 'sc', 2, dist, 0)
#     # 注意，tc1与MATLAB的comagnitude1中求法不一致
#     tc1 = TaoC(ch3, Sprate[0])
#     tc2 = TaoC(ch6, Sprate[0])
#     TC = (tc1 + tc2) / 2  # 原代码笔误为(tc1+tc1)/2，此处按逻辑修正为(tc1+tc2)/2
#
#     # --------------------------
#     # 3. 计算P_ud_m（第3/6通道的平均最大位移）
#     # --------------------------
#     # 定义滤波系数（原MATLAB的B/A、B1/A1）
#     B = np.array([0.998335306039878, -1.99667061207976, 0.998335306039878])
#     A = np.array([1.0, -1.99666784087186, 0.996673383287657])
#     B1 = np.array([0.0412535372417203, 0.0825070744834407, 0.0412535372417203])
#     A1 = np.array([1.0, -1.34896774525279, 0.513981894219675])
#
#     # 处理第3通道（ud1→v1→disp1→d1）
#     ud1 = lfilter(B, A, ch3)  # 第一次滤波
#     ud1 = lfilter(B1, A1, ud1)  # 第二次滤波
#     v1 = Simpson1(ud1)  # 一次积分（速度）
#     disp1 = Simpson1(v1)  # 二次积分（位移）
#     d1 = np.max(np.abs(disp1))
#     d1 = max(d1, np.finfo(float).eps)  # 避免d1为0（原代码eps）
#
#     ch6 = Traces_evt[:, 5]
#     ud2 = lfilter(B, A, ch6)
...
#     d2 = max(d2, np.finfo(float).eps)
#
#     # 计算P_ud_m（平均最大位移）
#     P_ud_m = (d1 + d2) / 2
#
#     # --------------------------
#     # 4. 计算sAvghm（4个通道的平均最大位移）
#     # --------------------------
#
#     # 定义滤波系数（原MATLAB的b/a）
#     b = np.array([
#         0.00474269258212374, 0.0, -0.0189707703284950, 0.0,
#         0.0284561554927425, 0.0, -0.0189707703284950, 0.0,
#         0.00474269258212374
#     ])
#     a = np.array([
#         1.0, -6.37105991085322, 17.8055507425771, -28.5714925148618,
#         28.8413192874792, -18.7736270033079, 7.69689004151592,
#         -1.81659872193766, 0.189018079395883
#     ])
#
#     # 对4个通道进行滤波（原MATLAB的EW1F/EW2F/NS1F/NS2F）
#     EW1F = lfilter(b, a, Traces_evt[:, 0])  # 第1通道（EW1）
#     EW2F = lfilter(b, a, Traces_evt[:, 3])  # 第4通道（EW2）
#     NS1F = lfilter(b, a, Traces_evt[:, 1])  # 第2通道（NS1）
#     NS2F = lfilter(b, a, Traces_evt[:, 4])  # 第5通道（NS2）
#
#     # 积分计算位移（调用srs_integration，原代码参数Distance→dist，推测为笔误）
#     displaceEW1 = srs_integration(EW1F, Sprate, dist)
#     displaceEW2 = srs_integration(EW2F, Sprate, dist)
#     displaceNS1 = srs_integration(NS1F, Sprate, dist)
#     displaceNS2 = srs_integration(NS2F, Sprate, dist)
#
#     # 计算各通道的最大绝对位移
