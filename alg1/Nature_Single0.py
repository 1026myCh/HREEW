...
def Nature_Single(Data_now, StartT, k, MaxEEW_times, StationInfo, NewInfo, Debug, Sprate, ThreshGals, S_time2,
                  Buffer_len, EEW_Time_After_S, Pspeed, Sspeed, Alarm, Sta_vars1, Sta_vars2, Sta_vars_ret1,
                  Sta_vars_ret2, Alarm1, Alarm2,flagarea,in_model,Gain):
    global model_Pwave
    global model_Swave


    if persistent.FirstStart == 1:
        model_Pwave = torch.load(os.path.join(os.getcwd(), 'china.rnn.jit'),
                                 map_location=torch.device('cpu'))  # axis=1
        # torch.load.
        model_Swave = torch.load(os.path.join(os.getcwd(), 'rnn.pnsn.01.jit'),
                                 map_location=torch.device('cpu'))  # axis=1
    ...
    if Sta_vars1.Is_EQK < 1 or Sta_vars2.Is_EQK < 1:
        result = Judges(Traces_evt, 'multi_parameters')
    ...
    if persistent.flag_nature == 1:
        ...
        if Sta_vars1.Magnitude <= 0:
            if Sta_vars1.S_time - Sta_vars1.S_timeold > 0:  ## &&Sta_vars1.S_timeold <= 0
                flag_Sfirst = 1
            Mag = cal_magnitude(Traces_evt, Distance, 0, Sprate,Debug, Sta_vars1.S_time, flagarea
            ...
            if Sta_vars1.S_time - Sta_vars1.S_timeold > 0:  ## & & Sta_vars1.S_timeold <= 0
                flag_Sfirst = 1
            Mag = cal_magnitude(Traces_evt, Distance, Sta_vars1.Magnitude, Sprate,Debug, Sta_vars1.S_time, flagarea
                                , Sta_vars1.PGAcurrold, PGAnew, flag_Sfirst, in_model, Gain, flag=None)
            Mag = np.round(Mag, 1)
            if Mag>7.5:
                Mag=7.5
