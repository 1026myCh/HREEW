...
def Azimuth3(datain):
    ...
    if len1>600:
        tempData = np.array(datain[:][0:601])
    else:
        tempData = np.array(datain)



    dataout10 = iomega_azi1(tempData[0],200,1)   # speed
    dataout11 = iomega_azi1(tempData[1], 200, 1) # speed
    dataout12 = iomega_azi1(tempData[2], 200, 1)  # speed
    ## 用位移代替测效果
    dataout10 = iomega_azi1(tempData[0], 200, 1)  #
...
def first_half_wave_length(signal):
    ...
    zero_crossings = np.where(np.diff(np.sign(signal)))
    peak = np.where(np.diff(np.sign(temp_signal)))[0]
    len=np.size(np.array(signal))
    peak1 =np.array(np.where(peak>zero_crossings[0][0]))
    rising_edge_index=zero_crossings[0][0]
    len_zeroc=np.size(zero_crossings[0])
    if peak1.size==1 :
        falling_edge_index=len
    else:
        if len_zeroc>1:
            if zero_crossings[0][1]>peak[peak1[0][1]]:
                falling_edge_index = peak[peak1[0][1]]
            else:
                falling_edge_index = zero_crossings[0][1]
        elif len_zeroc==1:
            falling_edge_index=peak[peak1[0][1]]
